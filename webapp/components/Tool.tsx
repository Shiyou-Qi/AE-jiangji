...
export default function Tool({
  ...
  useEffect(() => {
    const t0 = window.setTimeout(() => setStarted(true), 30);
  ...
  return (
    <main
      className="relative min-h-screen overflow-hidden text-[#f1f3f9]"
    ...
    >
      ...
      <div className="relative z-10 flex min-h-screen flex-col items-center px-5 pb-16 pt-12 sm:px-8">
        <div
          className={`w-full max-w-[720px] transition-all duration-700 ${
            started ? 'translate-y-0 opacity-100' : 'translate-y-5 opacity-0'
          }`}
        >
          {/* ===== 顶部徽章 + 语言切换 ===== */}
          <div className="mb-7 flex flex-wrap items-center justify-center gap-3">
            <span className="glass inline-flex items-center gap-2 rounded-full px-3.5 py-1.5 text-[11px] font-medium tracking-[0.18em] text-[#9fb0ff]">
          ...
          <header className="text-center">
            ...
            <p className="mx-auto mt-5 max-w-[480px] text-sm leading-relaxed text-[#a7adc0]">
              {t.heroSubBefore}{' '}
              <span className="text-white/80">.aep</span> {t.heroSubAfter}
            </p>
          </header>
...
function StepLabel({ n, title }: { n: string; title: string }) {
  return (
    <div className="flex items-center gap-3">
      <span className="flex h-6 w-6 items-center justify-center rounded-md border border-indigo-400/30 bg-indigo-400/10 text-[11px] font-bold tabular-nums text-indigo-300">
        {n}
      </span>
      <h2 className="text-sm font-bold tracking-[0.22em] text-white/90">{title}</h2>
...
function Feature({
  ...
  return (
    <div className="glass group rounded-2xl px-4 py-5 transition-all duration-300 hover:-translate-y-0.5 hover:border-white/20">
      <div
